<x-app-layout>
    <!-- Breadcrumb Start -->
    <div x-data="{ pageName: `Tambah Kasir` }">
                ...
                <form action="{{ route('admin.cashier.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" placeholder="Nama"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden "
                            value="{{ old('name') }}" />
                        @error('name')
                    ...
                    <div>
                        ...
                        <div x-data="{ showPassword: false }" class="relative">
                            <input :type="showPassword ? 'text' : 'password'" name="password" placeholder="Password"
                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 pl-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden " value="{{ old('password') }}"/>
                            @error('password')
                                <span class="text-error-500 text-sm">{{ $message }}</span>
                            @enderror
                    ...
                    <div>
                        ...
                        <input type="file" name="images" placeholder="images"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden " />
                        @if(old('images'))
                            <span class="text-sm text-gray-500">File sebelumnya: {{ old('images') }}</span>
                        @endif
                        @error('images')
                            <span class="text-error-500 text-sm">{{ $message }}</span>
                        @enderror
